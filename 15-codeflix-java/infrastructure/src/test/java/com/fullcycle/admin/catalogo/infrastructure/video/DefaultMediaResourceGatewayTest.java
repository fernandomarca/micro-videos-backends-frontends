...
class DefaultMediaResourceGatewayTest {
  ...
  public void givenValidResource_whenCallsStorageAudioVideo_shouldStoreIt() {
    ...
    // when
    final var actualMedia = this.mediaResourceGateway.storeAudioVideo(expectedVideoId,
  ...
  public void givenValidResource_whenCallsStorageImage_shouldStoreIt() {
    ...
    // when
    final var actualMedia = this.mediaResourceGateway.storeImage(expectedVideoId,
        VideoResource.with(expectedType, expectedResource));
  ...
  public void givenValidVideoId_whenCallsGetResource_shouldReturnIt() {
    ...
    storageService().store("videoId-%s/type-%s".formatted(videoOne.getValue(), VideoMediaType.TRAILER.name()),
        resource(mediaType()));
    storageService().store("videoId-%s/type-%s".formatted(videoOne.getValue(), VideoMediaType.BANNER.name()),
        resource(mediaType()));
  ...
  public void givenInvalidType_whenCallsGetResource_shouldReturnEmpty() {
    ...
    storageService().store("videoId-%s/type-%s".formatted(videoOne.getValue(), VideoMediaType.VIDEO.name()),
        resource(mediaType()));
    storageService().store("videoId-%s/type-%s".formatted(videoOne.getValue(), VideoMediaType.TRAILER.name()),
        resource(mediaType()));
    storageService().store("videoId-%s/type-%s".formatted(videoOne.getValue(), VideoMediaType.BANNER.name()),
