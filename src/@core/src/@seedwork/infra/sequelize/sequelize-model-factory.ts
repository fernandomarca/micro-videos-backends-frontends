...
export class SequelizeModelFactory<ModelClass extends Model, ModelProps = {}> {
  private _count = 1;
  constructor(
    private model,
  ...
  async bulkCreate(factoryProps?: (index: number) => ModelProps): Promise<ModelClass[]> {
    const data = new Array(this._count).fill(factoryProps ? factoryProps : this.defaultFactoryProps)
